execute store result score @s ne_table_interact run data get entity @s interaction.timestamp
execute unless score @s ne_table_interact matches 2147483647 run return -1

execute store result score #calc_high ne run data get entity @s interaction.timestamp 0.0001
data modify storage ne:tmp time_data.low set string entity @s interaction.timestamp -5 -1
