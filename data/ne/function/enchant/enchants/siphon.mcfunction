execute if data entity @s item.components."minecraft:stored_enchantments"."nel:siphon" run function ne:enchant/enchants_store/siphon
execute unless data entity @s item.components."minecraft:enchantments"."nel:siphon" run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:siphon"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:siphon"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:siphon" int 1 run scoreboard players get #ench_origin_level ne