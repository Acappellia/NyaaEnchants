execute if data entity @s item{id:"minecraft:enchanted_book"} run function ne:enchant/enchants_store/accuminate

execute store result score #invalid_enchant ne run function ne:enchant/enchants_check_valid/accuminate
execute if score #invalid_enchant ne matches ..-1 run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:accuminate"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:accuminate"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:accuminate" int 1 run scoreboard players get #ench_origin_level ne