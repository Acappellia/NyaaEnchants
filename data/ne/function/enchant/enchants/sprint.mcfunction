execute if data entity @s item.components."minecraft:stored_enchantments"."nel:sprint" run function ne:enchant/enchants_store/sprint
execute unless data entity @s item.components."minecraft:enchantments"."nel:sprint" run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:sprint"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:sprint"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:sprint" int 1 run scoreboard players get #ench_origin_level ne