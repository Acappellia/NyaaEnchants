execute if data entity @s item.components."minecraft:stored_enchantments"."nel:behead" run function ne:enchant/enchants_store/behead
execute unless data entity @s item.components."minecraft:enchantments"."nel:behead" run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:behead"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:behead"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:behead" int 1 run scoreboard players get #ench_origin_level ne