execute if data entity @s item.components."minecraft:stored_enchantments"."nel:sharpening" run function ne:enchant/enchants_store/sharpening
execute unless data entity @s item.components."minecraft:enchantments"."nel:sharpening" run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:sharpening"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:sharpening"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:sharpening" int 1 run scoreboard players get #ench_origin_level ne