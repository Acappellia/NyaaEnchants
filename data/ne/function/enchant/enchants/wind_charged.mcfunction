execute if data entity @s item.components."minecraft:stored_enchantments"."nel:wind_charged" run function ne:enchant/enchants_store/wind_charged
execute unless data entity @s item.components."minecraft:enchantments"."nel:wind_charged" run return -1

execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:enchantments"."nel:wind_charged"
execute store result score #ench_added_level ne run data get storage ne:tmp ench_component."minecraft:stored_enchantments"."nel:wind_charged"
...
execute store result entity @s item.components."minecraft:enchantments"."nel:wind_charged" int 1 run scoreboard players get #ench_origin_level ne