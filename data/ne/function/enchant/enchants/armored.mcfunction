execute if data entity @s item.components."minecraft:stored_enchantments"."nel:armored" run function ne:enchant/enchants_store/armored
execute unless data entity @s item.components."minecraft:enchantments"."nel:armored" run return -1
...
execute store result entity @s item.components."minecraft:enchantments"."nel:armored" int 1 run scoreboard players get #ench_origin_level ne