execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:stored_enchantments"."ne:supermending"
...
execute store result entity @s item.components."minecraft:stored_enchantments"."ne:supermending" int 1 run scoreboard players get #ench_origin_level ne