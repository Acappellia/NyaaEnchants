execute store result score #ench_origin_level ne run data get entity @s item.components."minecraft:stored_enchantments"."nel:brisk_walking"
...
execute store result entity @s item.components."minecraft:stored_enchantments"."nel:brisk_walking" int 1 run scoreboard players get #ench_origin_level ne